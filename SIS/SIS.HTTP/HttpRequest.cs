...
namespace SIS.HTTP
{
    public class HttpRequest
    {
        public HttpRequest(string httpRequestAsString)
        {
            ...
            var httpInfoHeaderParts = httpInfoHeader.Split(' ');
            if (httpInfoHeaderParts.Length != 3)
            {
                throw new HttpServerException("Invalid HTTP header line.");
            }

            var httpMethod = httpInfoHeaderParts[0];
            ...
            this.Path = httpInfoHeaderParts[1];

            var httpVersion = httpInfoHeaderParts[2];
